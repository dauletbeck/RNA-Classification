...
#        return Sphere(normals, sup_vec)
    ...
    def __fit_with_height (func, dim, initial, verbose):
        ...
        if tmp is None:
            return None
        height = sin(np.mean(np.arcsin(np.einsum('ij,j->i', points,
                                             tmp/la.norm(tmp)).clip(-1,1))))
    ...
    if mode is 'torus':
        ...
        if tmp is None:
            print('WARNING: Torus fit failed! Falling back to sphere fit.')
            tmp = __small_circle_fit()
        if verbose: print(la.norm(g(tmp)), la.norm(f(tmp[:-1])))
    elif mode is 'great':
    ...
    if mode in ['scale', 'torus']:
        ...
        if compare_likelihoods(radii, d-1, verbose):
            tmp2 = tmp.copy()
            tmp = fit((g2 if mode is 'torus' else f2), d, tmp[:-1], verbose, False)
            ang = np.arccos(np.einsum('i,i->', tmp, tmp2[:-1])) * DEG
            if verbose: print('CHANGED:', ang, abs(tmp2[-1]))
            return Sphere1(tmp / la.norm(tmp), 0)
    return Sphere1(tmp[:-1] / la.norm(tmp[:-1]), tmp[-1])

def pns_loop (points, great_until_dim,  max_repetitions, degenerate=False,
              verbose=False, mode=None, half=False):
    list_spheres = []
    list_points = []
    list_dists = []
...
def circular_mean (points):
    n = points.size
    mean0 = np.mean(points)
    var0 = np.var(points)
    sorted_points = np.sort(points)
    means = variances(mean0, var0, n, sorted_points)
    return means[np.argmin(means[:,1])]
...
def torus_mean_and_var(data):
    mean = []
    variance = 0
    for k in range(data.shape[1]):
        tmp = circular_mean(data[:,k])
        mean.append(tmp[0])
        variance += tmp[1]
...
def compare_likelihoods (radii, d, verbose):
    mean = radii.mean()
    std = radii.std()
    def likelihood (x):
        penalty = 0.
        ...
        if scale > 1:
            x[0] = 0.5 * PI / x[1]
            penalty = scale
        out = np.sum(log(normalization(x[0],x[1],d)) + 0.5*(radii/x[1]-x[0])**2 -
    ...
    mle = opt.minimize(likelihood, np.array([max(mean/std, 1.), std]),
                       ...
                       bounds=((0,PI*1e3),(max(1e-3,0.25*std),0.5*PI))).x
...
class Sphere1 (object):
    ...
    def calculate_projection_matrix (self):
        self.projection_matrix = np.eye(len(self.normal))
        self.projection_matrix[0] = self.normal
...
class Sphere (object):
    ...
    def calculate_projection_matrix (self):
        self.projection_matrix = np.eye(self.normals.shape[1])
        self.projection_matrix[:len(self.normals)] = self.normals
