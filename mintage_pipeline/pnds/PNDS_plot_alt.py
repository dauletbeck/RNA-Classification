...
def scatter_plots (data, filename = None):
    fig = plot.figure()
    n = data.shape[1]
...
def colored_scatter_plots_flex_range (data, n0, n1, filename = None):
    ...
    diff = 0.05 * (hi - lo); lo -= diff; hi += diff
    for y in range(n0,n1):
        for x in range(n0,y):
            diag = fig.add_subplot(n-1,n-1,1+x-n0+(n-1-y+n0)*(n-1))
            diag.set_title(r'$x = \alpha_{' + str(x+1) +
...
def residual_plots (data, scale, n, colors, filename = None):
    fig = plot.figure()
    size = fig.get_size_inches()
...
def make_histogram (data, filename = None):
    ...
    rounded = np.round(data / bin_size).astype(np.int)
...
def unique_list (l):
    unique = set(); unique_add = unique.add
